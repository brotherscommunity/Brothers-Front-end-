...
export default function EditProfile({ userData }: EditProfileProps) {
  ...
  async function onSubmit(values: z.infer<typeof EditProfileSchema>) {
    ...
    const { userName, file, newPassword, confirmPassword, languages, ...rest } =
      values;
    const languagesList = languages.split(",");
    execute({ password: newPassword, ...rest, languages: languagesList });
  }
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
              ...
              <FormItem>
                <FormControl>
                  <AvatarUploader
                    ...
                    mediaUrl="/Images/profile-uploader.png"
                    username={userData?.userName!}
                  />
                </FormControl>
          ...
          <div className="flex justify-center mt-20">
            ...
            >
              {isLoading ? <Spinner loading={isLoading} /> : "Save Changes"}{" "}
            </button>
          </div>
        </form>
      </Form>
